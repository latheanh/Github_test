...
namespace WindowsFormsApp1
{
    public partial class frmDMChatlieu : Form
    {
        ...
        private void LoadDataGridView()
        {
            ...
            tblChatLieu = QuanLyBanHang.Class.Functions.GetDataToTable(sql); //Đọc dữ liệu từ bảng
            dgvChatLieu.DataSource = tblChatLieu; //Nguồn dữ liệu            
            dgvChatLieu.Columns[0].HeaderText = "Mã loại hàng"; // Mã chất liệu
            dgvChatLieu.Columns[1].HeaderText = "Tên loại hàng"; // Tên chất liệu
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Bạn phải nhập mã loại hàng" , "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information); // mã chất liệu
                txtMaChatLieu.Focus();
                return;
            }
            if (txtTenChatLieu.Text.Trim().Length == 0) //Nếu chưa nhập tên chất liệu
            {
                MessageBox.Show("Bạn phải nhập tên loại hàng", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information); //tên chất liệu
                txtTenChatLieu.Focus();
                return;
            }
            sql = "Select MaChatLieu From tblChatLieu where MaChatLieu=N'" + txtMaChatLieu.Text.Trim() + "'";
            if (QuanLyBanHang.Class.Functions.CheckKey(sql))
            {
            ...
            QuanLyBanHang.Class.Functions.RunSQL(sql); //Thực hiện câu lệnh sql
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            QuanLyBanHang.Class.Functions.RunSQL(sql);
            LoadDataGridView();
            ResetValue();
        ...
        {
            {
                ...
                if (MessageBox.Show("Bạn có muốn xoá không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    QuanLyBanHang.Class.Functions.RunSqlDel(sql);
                    LoadDataGridView();
                    ResetValue();
                }
            }
        ...
        private void frmDMChatlieu_Click(object sender, EventArgs e)
        {
            ...
            if (tblChatLieu.Rows.Count == 0) //Nếu không có dữ liệu
            {
